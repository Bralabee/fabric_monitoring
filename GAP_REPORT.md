...
- Packaging uses `pyproject.toml` with `src/` layout; dependencies are declared and version is consistent (`0.3.36`).
...
1) Environment setup documentation contradicts the actual auth implementation
...
1) `.env.template` appears copied from a different repository and is misleading
...
1) Script UX messaging has contradictions
