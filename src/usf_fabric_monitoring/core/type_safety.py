...
def safe_int64(value: Any) -> int | None:
    """
...
def coerce_surrogate_keys(df: pd.DataFrame, inplace: bool = False) -> pd.DataFrame:
    ...
    for col in sk_columns:
        if col in df.columns:
            ...
            try:
                df[col] = pd.array([safe_int64(v) for v in df[col]], dtype=pd.Int64Dtype())
...
def ensure_columns_exist(df: pd.DataFrame, columns: list[str], fill_value: Any = None) -> pd.DataFrame:
    """
